...
public class Querys {
		...
		try {
			...
			while (iterador.hasNext()) {
				...
				if (parametro.getTipoOperacion().equalsIgnoreCase("I")
					...
					Object bean = pm.getObjectById(parametro.getBean()
							.getClass(), parametro.getId());
					objectsE.add(bean);
				} else {
	...
	public boolean mantenimiento(BeanParametro parametro)
		...
		try {
			if (parametro.getTipoOperacion().equalsIgnoreCase("I")) {
				...
				Object bean = pm.getObjectById(parametro.getBean().getClass(),
						parametro.getId());
	...
	@SuppressWarnings({ "rawtypes", "unchecked" })
	public Collection<?> getListaBean(Class<?> nomClase)
			throws UnknownException {
		try {
